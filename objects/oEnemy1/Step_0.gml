...
switch (state) {
	...
	case states.circle: // Circle state
		...
		// Shoot
		actionDur++
		if(actionDur >= 0.2 * room_speed){							// Creates an bullet aim at the player's current location
			actionDur = 0;
			bullet = instance_create_depth(x, y, 1, oEnemyBullet);
			audio_play_sound(Shoot, 1, 0);
			bullet.direction = direction;
...
if(hp <= 0){
	...
	// Dropping loot
	var drop = irandom(9);
	if (drop == 0 || drop == 1) {							// 2/10 chance to drop a heart
